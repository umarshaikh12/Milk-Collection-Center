...
function processDetails($con, $farmerName, $milkType) {
    ...
    // Calculate mean fat
    if ($rowCount > 0) {
        $meanFat = $totalFat / $rowCount;
    }

    // Display the total milk for the farmer
    echo "<tr class='total'><td>Total Milk</td><td>$totalMilk</td><td></td><td></td></tr>";

    // Display the mean fat for the farmer
    echo "<tr class='total'><td>Mean Fat</td><td>$meanFat</td><td></td><td></td></tr>";

    // Fetch fat price from fat_prices table
    $sqlFatPrice = "SELECT price FROM fat_prices WHERE $meanFat BETWEEN fat_from AND fat_to AND milk_type = '$milkType'";
    $resultFatPrice = $con->query($sqlFatPrice);

    if ($resultFatPrice === FALSE) {
        die("Error fetching fat price: " . $con->error);
    }

    // Check if a valid fat price is found
    if ($resultFatPrice->num_rows > 0) {
        $fatPriceRow = $resultFatPrice->fetch_assoc();

        // Extract price
        $price = $fatPriceRow["price"];

        // Calculate total payment
        $totalPayment = $totalMilk * $price;

        // Display the total payment for the farmer
        echo "<tr class='total'><td>Total Payment</td><td>$totalPayment</td><td></td><td></td></tr>";
    } else {
        // Display a message if no valid fat price is found
        echo "<tr class='total'><td colspan='4'>No valid fat price found.</td></tr>";
    }
